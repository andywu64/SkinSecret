...
enum class TipTopLevelDestination(
    ...
    Product(
        ...
        iconText = "產品",
        route = getProductNavigationRoute(),
        title = R.string.product
    ),
    ProductDetail(
        icon = Icons.Filled.Home,
        iconText = "詳細資料",
        route = getProductDetailNavigationRoute(),
...
class ProductAppState(
    ...
    private fun getTopLevelDestination(route: String?): TipTopLevelDestination? {
        return when (route) {
            getProductNavigationRoute() -> TipTopLevelDestination.Product
            getProductDetailNavigationRoute() -> TipTopLevelDestination.ProductDetail
            else -> null
        }
    }
    ...
    val isShowBottomBar: Boolean
        @Composable get() {
            return when (currentDestination) {
                getProductNavigationRoute() -> true
                else -> false
            }
        }
}
